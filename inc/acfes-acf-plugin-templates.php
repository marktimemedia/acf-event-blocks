...
add_filter('acf/location/rule_types', 'acfes_location_rules_blocktypes');
function acfes_location_rules_blocktypes( $choices )
{
...
add_filter('acf/location/rule_values/blocktemplates', 'acfes_location_rules_values_blocktemplates');
function acfes_location_rules_values_blocktemplates( $choices )
{
	$choices = array();
...
add_filter('acf/location/rule_match/blocktemplates', 'acfes_location_rules_match_blocktemplates', 10, 3);
function acfes_location_rules_match_blocktemplates( $match, $rule, $options )
{
    // bail early if not a post
		...
		// vars
		$templates = array();
		$acfes_post_type = get_post_type( $options['post_id'] );
		...
		if( !isset($templates[ $acfes_post_type ]) ) return false;
